...
const ujianaktifkeywords = [ 'aktif', 'berlaku', 'berjalan', 'berlangsung' ];
const ujiannonaktifkeywords = [ 
...
module.exports = {
  ...
  const id = /^[1-9]\d{0,2}(?:\,\d{1,3})?$/.test(find);
    if (ujianaktifkeywords.some(v => find.includes(v))) {        
      filtered = 1
      searchObj = [        
        {aktif: {[Op.eq]: filtered }}
      ]
    } else if (ujiannonaktifkeywords.some(v => find.includes(v))) {
      filtered = 0
      searchObj = [        
        {aktif: {[Op.eq]: filtered }}
      ]
